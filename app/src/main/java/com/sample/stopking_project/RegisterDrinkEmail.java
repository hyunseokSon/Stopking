...
public class RegisterDrinkEmail extends AppCompatActivity {
    ...
    private String avgDrink, weekDrink, drinkBank;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        avgDrink = intent.getStringExtra("avgDrink");   // 한 번에 먹는 술 병
        weekDrink = intent.getStringExtra("weekDrink"); //일주일에 마시는 술 횟수
        drinkBank = intent.getStringExtra("drinkBank"); // 금주저금통
        ...
        mbtnComplete.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                {
                    //email 입력을 안 했을 경우.
                    Toast.makeText(RegisterDrinkEmail.this, "이메일 주소를 입력해주세요.", Toast.LENGTH_SHORT).show();
                }
                else if(mEtPwd.getText().toString().equals("") || mEtPwd.getText().toString() == null)
                {
                    //비밀번호 입력을 안 했을 경우.
                    Toast.makeText(RegisterDrinkEmail.this, "비밀번호를 입력해주세요.", Toast.LENGTH_SHORT).show();
                }
                else if (mEtPwd.getText().toString().length() < 6)
                {
                    //비밀번호가 6자리 이하일 경우
                    Toast.makeText(RegisterDrinkEmail.this, "비밀번호를 최소 6글자 이상 입력해주세요.", Toast.LENGTH_SHORT).show();
                }
                else
                {
                    ...
                    // 일주일에 마시는 총 병수
                    String intAvgDrink = avgDrink.replaceAll("[^0-9]", "");
                    int weekBottle = Integer.parseInt(weekDrink) * Integer.parseInt(intAvgDrink);

                    mAuth.createUserWithEmailAndPassword(strEmail, strPwd).addOnCompleteListener(RegisterDrinkEmail.this, new OnCompleteListener<AuthResult>() {
                        @Override
                    ...
                    //금주 정보
                    user.put("average_drink", intAvgDrink);
                    ...
                    db.collection("users").document(strEmail).set(user).addOnCompleteListener(new OnCompleteListener<Void>() {
                            ...
                            .addOnFailureListener(new OnFailureListener() {
                                ...
                                public void onFailure(@NonNull Exception e) {
                                    ...
                                    Toast.makeText(RegisterDrinkEmail.this, "회원가입에 실패하였습니다.", Toast.LENGTH_SHORT).show();
                                }
                            });
                }
            }
